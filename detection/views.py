...
def serve_media_file(request: HttpRequest, file_path: str) -> HttpResponse:
    ...
    R2 mode (USE_R2=True): redirects to a presigned S3 URL generated by
    the R2 storage backend — nginx is not involved.

    Local mode (USE_R2=False): delegates to nginx via X-Accel-Redirect.
    """
    from django.conf import settings as django_settings

    # Sanitize path — prevent directory traversal in both modes
    ...
    # Local storage — delegate actual byte-serving to nginx
    ...
    response["Content-Type"] = ""  # Let nginx determine MIME type
    return response
